...
function Patient({ showCreateForm, setShowCreateForm, showUpdateForm, setShowUpdateForm, setSelectedPatientId }) {
    ...
    useEffect(() => {
        if (!location.state?.openUpdateForm || patients.length === 0 || hasOpenedForm.current) return;
        ...
        const checkAndOpenUpdateForm = async () => {
            try {
                ...
                const matchingPatient = patients.find(patient => patient.Email === currentUserEmail);
                if (matchingPatient) {
                    setSelectedPatientId(matchingPatient.Patient_ID);
                    setShowUpdateForm(true);
    
                    hasOpenedForm.current = true; // ✅ Prevent further executions
    
                    setTimeout(() => {
                        navigate(location.pathname, { replace: true, state: {} });
                    }, 100); // ✅ Small delay to allow state updates
                } else {
                    console.warn("No patient record found for email:", currentUserEmail);
                }
            } catch (err) {
    ...
    }, [location.state, patients]); // ✅ Keeps dependencies but prevents infinite execution
    ...
    const handleDeleteConfirm = async () => {
        try {
            await axios.delete(`http://localhost:9004/api/patient/delete/${deletePatientId}`, { withCredentials: true });
    ...
    const columns = [
        ...
        { field: 'Joining_Date', headerName: 'Joining_Date', flex: 1 },
