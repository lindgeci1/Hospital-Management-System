...
import { Box, TextField, Button, FormHelperText, Typography, Grid, InputLabel, MenuItem, Select, FormControl, Modal } from '@mui/material';
...
function UpdateVisit({ id, onClose }) {
        ...
        const fetchData = async () => {
            try {
                const [visitRes, patientRes, doctorRes] = await Promise.all([
                    axios.get(`http://localhost:9004/api/visit/${id}`, { withCredentials: true }),
                    axios.get('http://localhost:9004/api/patient', { withCredentials: true }),
                    axios.get('http://localhost:9004/api/doctors', { withCredentials: true })
                ]);
    ...
    const fetchDoctorQualifications = async (doctorId) => {
        try {
            const response = await axios.get(`http://localhost:9004/api/doctors/${doctorId}`, { withCredentials: true });
    ...
    const handleUpdateVisit = async () => {
        ...
        try {
            const response = await axios.put(`http://localhost:9004/api/visit/update/${id}`, formData, { withCredentials: true });
...
<Modal open onClose={onClose} className="fixed inset-0 flex items-center justify-center z-10 bg-black/50">
    <Box sx={{ bgcolor: 'background.paper', p: 4, borderRadius: 2, width: 600, mx: 'auto' }}>
        ...
        <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
                ...
                <TextField
                    ...
                    value={patientPhone}
                    readOnly
                    helperText="This is the phone number of the patient"
                />
            ...
            <Grid item xs={12} sm={6}>
                <FormControl fullWidth margin="dense" variant="outlined">
                    ...
                    <FormHelperText>Patient's condition during the visit</FormHelperText>
                </FormControl>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 2 }}>
            <Button variant="contained" color="primary" onClick={handleUpdateVisit} sx={{ mr: 1 }}>
                Submit
            </Button>
