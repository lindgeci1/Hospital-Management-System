...
export function Profile() {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (response.status === 200) {
          console.log("✅ User is authenticated. Preventing back to login.");
          ...
          // ✅ Prevent going back to `/login` AND prevent logout from spamming back
          window.history.pushState(null, "", window.location.href);
          window.addEventListener("popstate", preventBackToLogin);
        }
      } catch (err) {
        console.error('🔴 User not authenticated. Allowing navigation to login.', err);
      }
    };
  ...
  // ✅ Prevent the browser back button from logging out when spammed
  useEffect(() => {
