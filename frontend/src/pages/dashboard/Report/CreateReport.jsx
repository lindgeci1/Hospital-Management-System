...
const CreateReport = ({ onClose, onSaveSuccess }) => {
  ...
  const checkExistingReport = async (patientId) => {
    try {
        const response = await axios.get(`http://localhost:9004/api/reports/check/${patientId}`, { withCredentials: true });
...
useEffect(() => {
  const fetchPatientVisits = async () => {
    ...
    try {
      ...
      const visitCheckResponse = await axios.get(`http://localhost:9004/api/patient/${selectedPatient}/visit`, { withCredentials: true });
      ...
      const response = await axios.get(`http://localhost:9004/api/visit/patient/${selectedPatient}`, { withCredentials: true });
