...
function UpdateBill({ id, onClose }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:9004/api/bills/${id}`,{ withCredentials: true });
    ...
    const updateBill = async () => {
        try {
            const response = await axios.put(`http://localhost:9004/api/bills/update/${id}`, formData, { withCredentials: true });
            onClose();
