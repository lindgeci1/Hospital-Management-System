...
import { Box, TextField, Button, Typography, Grid, Select, FormHelperText, MenuItem, InputLabel, FormControl, Modal } from '@mui/material';
...
function CreateStaff({ onClose }) {
    ...
    const handleAddStaff = async () => {
        try {
            await axios.post("http://localhost:9004/api/staff/create", formData, { withCredentials: true });
    ...
    const handleValidation = async () => {
        ...
        if (!Personal_Number|| !Emp_Fname || !Emp_Lname  || !Email || !Dept_ID || !Birth_Date || !Gender|| !Phone|| !Qualifications || !Specialization) {
            showAlert('All fields are required3!');
            return;
        }
    ...
    return (
        <Modal open onClose={onClose} className="fixed inset-0 flex items-center justify-center z-10 bg-black/50">
            <Box sx={{ bgcolor: 'background.paper', p: 3, borderRadius: 2, width: 600, mx: 'auto' }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-end'}}>
                    <Button variant="contained" color="primary" onClick={handleValidation} sx={{ mr: 0.5 }}>
                        Submit
                    </Button>
