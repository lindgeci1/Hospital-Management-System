...
function UpdateDepartment({ id, onClose }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:9004/api/department/${id}`, { withCredentials: true });
    ...
    const handleUpdateDepartment = async () => {
        ...
        try {
            await axios.put(`http://localhost:9004/api/department/update/${id}`, formData, { withCredentials: true });
        ...
        <Modal open onClose={onClose} className="fixed inset-0 flex items-center justify-center z-10 overflow-auto bg-black bg-opacity-50">
            <Box sx={{ bgcolor: 'background.paper', p: 4, borderRadius: 2, width: 400, mx: 'auto' }}>
                ...
                <TextField
                    ...
                    disabled
                    helperText="Name of the department head (at least 2 characters)."
                />
    ...
    <FormHelperText>Enter department name (cannot contain numbers).</FormHelperText>
</FormControl>
